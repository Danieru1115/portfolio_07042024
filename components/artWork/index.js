        ...
        <div className='w-full h-[375px] overflow-x-hidden mt-4'>
          <div ref={marqueeRef} className={'w-max gap-8 flex animate-marquee2'}>
            {ArtWorkInfo.map((item, index) => (
                <a href={item.link} target="_blank" rel="noopener noreferrer" className=' group w-80 h-auto bg-white rounded-2xl border border-solid border-divider1 p-4 cursor-pointer'>
                    ...
                    <h1 className=' text-black3 text-base font-semibold'>{item.title}</h1>
                </a>
            ))}
          </div>
        </div>
